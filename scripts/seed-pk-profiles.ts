...
const PK_PROFILES: PkSeed[] = [
  // ── GLP-1 / Metabolic ─────────────────────────────────────────────────────
  { name: 'Semaglutide', aliases: ['Ozempic', 'Wegovy'], half_life_hours: 168, tmax_hours: 24, category: 'glp1', notes: 'Werte fuer die subkutane Gabe. Die Tablette (Rybelsus) hat unter einem Prozent Bioverfuegbarkeit und braucht ein eigenes Profil — diese Kurve passt nicht auf sie.' },
  { name: 'Tirzepatide', aliases: ['Mounjaro', 'Zepbound'], half_life_hours: 120, tmax_hours: 8, category: 'glp1', notes: 'Fachinformation, subkutan. Subkutan injiziert, daher volle Peak-Skalierung.' },
  { name: 'Liraglutide', aliases: ['Victoza', 'Saxenda'], half_life_hours: 13, tmax_hours: 8, category: 'glp1', notes: 'Fachinformation, subkutan. Subkutan injiziert, daher volle Peak-Skalierung.' },
  { name: 'Exenatide', aliases: ['Byetta'], half_life_hours: 2.4, tmax_hours: 2, category: 'glp1', notes: 'Werte fuer die schnell freisetzende Form (Byetta). Die Wochendepot-Form (Bydureon) verlaeuft voellig anders.' },
  ...
  // ── Research-Peptide ──────────────────────────────────────────────────────
  { name: 'BPC-157', aliases: ['Body Protection Compound'], half_life_hours: 4, tmax_hours: 0.5, category: 'peptide', notes: 'Im Serum tatsaechlich nur Minuten nachweisbar; der Wert hier bildet die laenger anhaltende Wirkung ab, nicht eine gemessene Elimination. Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung.' },
  { name: 'TB-500', aliases: ['Thymosin Beta-4'], half_life_hours: 72, tmax_hours: 2, category: 'peptide', notes: 'Die 72 Stunden sind ein in der Anwenderliteratur gebraeuchlicher Wert, keine belastbare Messung am Menschen. Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung.' },
  { name: 'GHK-Cu', aliases: ['Copper Peptide'], half_life_hours: 1, tmax_hours: 0.5, category: 'peptide', notes: 'Sehr kurz im Blut. Subkutan; topisch aufgetragen gilt die Kurve nicht.' },
  { name: 'KPV', aliases: [], half_life_hours: 2, tmax_hours: 0.5, category: 'peptide', notes: 'Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung. Subkutan.' },
  { name: 'CJC-1295 DAC', aliases: ['CJC-1295 with DAC'], half_life_hours: 192, tmax_hours: 2, category: 'peptide', notes: 'Die acht Tage kommen von der Bindung an Albumin (Drug Affinity Complex) — ohne DAC ist es ein anderer Stoff mit Minuten statt Tagen.' },
  { name: 'CJC-1295 no DAC', aliases: ['Modified GRF 1-29'], half_life_hours: 0.5, tmax_hours: 0.25, category: 'peptide', notes: 'Ohne Albuminbindung im Minutenbereich. Wirkt ueber die ausgeloeste GH-Ausschuettung, die laenger anhaelt als der Spiegel.' },
  { name: 'Ipamorelin', aliases: [], half_life_hours: 2, tmax_hours: 0.25, category: 'peptide', notes: 'Kurz im Blut; die ausgeloeste GH-Ausschuettung haelt laenger an als die Kurve zeigt. Subkutan.' },
  { name: 'GHRP-2', aliases: ['Pralmorelin'], half_life_hours: 1, tmax_hours: 0.25, category: 'peptide', notes: 'Wie Ipamorelin: kurz im Blut, laengere GH-Antwort. Subkutan.' },
  { name: 'GHRP-6', aliases: [], half_life_hours: 2, tmax_hours: 0.25, category: 'peptide', notes: 'Wie Ipamorelin: kurz im Blut, laengere GH-Antwort. Subkutan.' },
  ...
  { name: 'Sermorelin', aliases: [], half_life_hours: 0.17, tmax_hours: 0.1, category: 'peptide', notes: 'Sehr kurz im Blut: Gipfel und Halbwertszeit liegen beide im Minutenbereich. Die ausgeloeste GH-Ausschuettung haelt deutlich laenger an.' },
  ...
  { name: 'MK-677', aliases: ['Ibutamoren', 'Nutrobal'], half_life_hours: 6, tmax_hours: 1, category: 'peptide', bioavailability_sc: 0.6, notes: 'Oral, nicht injiziert — Bioverfuegbarkeit rund 60 Prozent. Kein Peptid, sondern ein kleines Molekuel; steht hier nur der Nachbarschaft wegen unter den Peptiden.' },
  ...
  { name: 'Semax', aliases: [], half_life_hours: 0.5, tmax_hours: 0.25, category: 'peptide', notes: 'Intranasal oder subkutan; sehr kurz im Blut, die Wirkung im Gehirn haelt laenger an. Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung.' },
  { name: 'Selank', aliases: [], half_life_hours: 1, tmax_hours: 0.25, category: 'peptide', notes: 'Intranasal oder subkutan; sehr kurz im Blut. Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung.' },
  { name: 'Dihexa', aliases: [], half_life_hours: 24, tmax_hours: 2, category: 'peptide', notes: 'Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung. Die Werte stammen aus Tierversuchen.' },
  { name: 'Cerebrolysin', aliases: [], half_life_hours: 1, tmax_hours: 0.5, category: 'peptide', notes: 'Ein Gemisch aus vielen Peptidfragmenten, keine einzelne Substanz — eine gemeinsame Halbwertszeit gibt es streng genommen nicht. Die Kurve ist eine grobe Naeherung.' },
  { name: 'NA-Semax Amidate', aliases: [], half_life_hours: 0.75, tmax_hours: 0.25, category: 'peptide', notes: 'Intranasal. Laenger stabil als Semax selbst, aber ebenfalls ohne belastbare Messung am Menschen.' },
  { name: 'PT-141', aliases: ['Bremelanotide'], half_life_hours: 2.7, tmax_hours: 1, category: 'peptide', notes: 'Subkutan oder intranasal. Subkutan injiziert, daher volle Peak-Skalierung.' },
  ...
  { name: 'Epithalon', aliases: ['Epitalon'], half_life_hours: 1, tmax_hours: 0.5, category: 'peptide', notes: 'Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung. Subkutan.' },
  { name: 'SS-31', aliases: ['Elamipretide'], half_life_hours: 2, tmax_hours: 0.5, category: 'peptide', notes: 'Reichert sich in den Mitochondrien an — der Blutspiegel sagt wenig darueber, wie lange der Stoff am Wirkort bleibt. Subkutan.' },
  { name: 'Humanin', aliases: [], half_life_hours: 3, tmax_hours: 1, category: 'peptide', notes: 'Duenne Datenlage beim Menschen — die Kurve zeigt eine Groessenordnung, keine Messung. Subkutan.' },
