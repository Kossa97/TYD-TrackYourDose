...
describe('My Stack page vial view', () => {
  ...
  test('gibt der Bühne, was übrig ist, statt einer geratenen Bildschirmhöhe', () => {
    ...
    expect(text).toContain("'calc(100dvh - env(safe-area-inset-bottom) - 298px)'")
    // 208 feste Teile plus 90 für die Navigationsleiste, wie sie stand, als
    // die Bühne eingestellt wurde — als feste Zahl und absichtlich NICHT als
    // `var(--bottom-nav-height)`: sonst wüchse die Bühne bei jedem Umbau der
    // Leiste mit, und My Stack sähe jedes Mal anders aus. Zweimal ist genau
    // das passiert.
    expect(208 + 90).toBe(298)
    expect(text).not.toContain("var(--bottom-nav-height) - env(safe-area-inset-bottom) - 2")
