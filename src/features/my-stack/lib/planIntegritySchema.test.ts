...
describe('My Stack plan-integrity confirmation contract', () => {
  ...
  it('limits editable decisions, timestamps and version provenance to exact pending rows', () => {
    ...
    expect(rpc).toMatch(/saved_log\.taken is not null and \([\s\S]*saved_log\.logged_at <> entry_logged_at[\s\S]*saved_log\.plan_version_id is distinct from expected_plan_version_id/)
    expect(rpc).toContain('saved_log.taken is distinct from entry_taken')
    expect(rpc).toContain("jsonb_typeof(entry -> 'taken') = 'boolean'")
