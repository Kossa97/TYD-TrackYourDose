...
describe('MyStackPage non-vial visibility', () => {
  ...
  it('keeps an active non-vial item visible and editable beside the premium vial stage', async () => {
    ...
    expect(screen.queryByTestId('stack-stage-patch-1')).toBeNull()
  ...
  it('summarizes a non-vial item with dosage form and ingredient strength', async () => {
    ...
    const card = visibleCardFor(qaName)
    expect(card).not.toBeNull()
    expect(card?.textContent).toContain('dosage_form_patch')
    expect(card?.textContent).toContain('Magnesium: 100 mg / 1 patch')
    expect(card?.textContent).toContain('Vitamin D3: 5000 IU / 1 patch')
  ...
  it('opens a PK deep link with the existing item and its active plan', async () => {
    ...
    render(
      <MemoryRouter initialEntries={['/my-stack?edit=patch-1&intent=pk']}>
        <LocationProbe />
    ...
    expect(screen.getByTestId('wizard-item-id').textContent).toBe('patch-1')
