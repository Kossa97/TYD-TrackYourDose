...
export function DosageFormCarousel({
  ...
  function helligkeitMessen(): void {
    ...
    for (const form of formen) {
      const objekt = objektRahmen(reihe, form.key)
      if (!objekt) continue
      helligkeiten[form.key] = fokusAusAbstand((objekt.left + objekt.width / 2 - mitte) / spanne)
    }
    setFokusJeForm(vorher => ({ ...vorher, ...helligkeiten }))
  }

  function helligkeitPlanen(): void {
  ...
  function scrollZu(key: DosageFormKey, sanft: boolean): void {
    ...
    window.requestAnimationFrame(helligkeitMessen)
  }
  ...
  // Erstes Bild: nur die Helligkeit setzen, nichts auswaehlen. Im naechsten
  // Frame statt sofort — ein synchrones setState im Effekt laeuft in eine
  // zweite Renderrunde, und gemessen werden kann ohnehin erst, wenn der
  // Browser die Reihe gelegt hat.
  useEffect(() => {
    const frame = window.requestAnimationFrame(helligkeitMessen)
  ...
  function beiScroll(): void {
    ...
    // Ruhe kommt, nicht bei jedem Zwischenbild.
    if (auswahlFrameRef.current !== null) window.cancelAnimationFrame(auswahlFrameRef.current)
  ...
  // ein `overflow-x-auto` nimmt davon nichts an. Eine Radumdrehung ruecht die
  ...
  function beiRad(ereignis: React.WheelEvent<HTMLDivElement>): void {
    ...
    if (index < 0) return
    const ziel = formen[Math.min(formen.length - 1, Math.max(0, index + (ereignis.deltaY > 0 ? 1 : -1)))]
    if (ziel && ziel.key !== mitte) scrollZu(ziel.key, true)
  ...
  const renderForm = (form: DosageFormDefinition) => {
    ...
    // steht das Objekt in seiner nativen Groesse.
    ...
    return (
      <button
        ...
        onClick={() => { if (!klickSperreRef.current) onSelect(form.key) }}
      ...
      >
        <span className="relative flex h-full w-full items-end justify-center px-2" aria-hidden="true">
          {/* Das Licht liegt UNTER dem Objekt, wie ein Spot auf der Buehne,
              ...
              sagt die Farbe im Objekt selbst. */}
          <span
          ...
          {isStageRenderable(form.key) ? (
            ...
            // derselben Linie stehen bleiben.
            <span
              className={`origin-bottom ${zieht ? '' : 'transition-transform duration-200 motion-reduce:transition-none'}`}
